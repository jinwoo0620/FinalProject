...
<div class="pc-container">
    <div class="pcoded-content">
        ...
        <div class="row">
            <!-- [ sample-page ] start -->
            <div class="col-sm-12">
            <form action="nupView4.no" method="post">
                <div class="card">
                    <div class="card-header">
                    
                        <div>
							<h3>${ notice4.noticeTitle4 }</h3>
							<input type="hidden" value="${ notice4.noticeTitle4 }" name="noticeTitle4">
							<input type="hidden" value="${ notice4.noticeNo4 }" name="noticeNo4">	
							<input type="hidden" value="${ page }" name="page">
						</div>
						<c:url var="ndelete4" value="ndelete4.no">
							<c:param name="nId" value="${ notice4.noticeNo4 }"/>
						</c:url>
                        <c:if test="${ loginUser.id eq notice4.writer4 }">
	                       <div align ="right">
		                        <button style="color: white" id = "button" type="submit" class="btn">수정</button>
		                        <button style="color: white" id = "button" type="button" class="btn" onclick="location.href='${ ndelete4 }'">삭제</button>
	                       </div>
                        </c:if>
                    </div>
                    <div class="card-body">
					...
					<div>
						${ fn:replace(notice4.noticeContent4, newLineChar, "<br>") }
						<input type="hidden" value="${ notice4.noticeContent4 }" name="noticeContent4">
					</div>
                    </div>
                 </div>
                 </form>
        ...
        <div class="card">
             <div class="card-header">
                        <h5>댓글<input type="text" class="form-control" id="replyContent4"></h5>
                        <br>
         ...
         <div class="na">
			<div class="menu" ><a href="nlist4.no" style = "color:white">이전으로</a></div>
		</div>
        ...
        <script>
        	 $('#rSubmit').click(function(){
        		var rContent = $('#replyContent4').val();
        		var refNId = ${notice4.noticeNo4};
        		
        		
        		$.ajax({
        			url: 'addReply4.no',
        			data: {replyContent4:rContent, boardNo4:refNId},
        		...
        		$.ajax({
        			...
        			success: function(data){
        				...
        				if(data.length > 0){
        					for(var i in data){
        						$tr = $('<tr>');
        						$memberId = $('<td>').css('width', '100px').text(data[i].memberId4);
        						$content = $('<td>').text(data[i].replyContent4);
        						$date = $('<td width="100px">').text(data[i].createDate4);
